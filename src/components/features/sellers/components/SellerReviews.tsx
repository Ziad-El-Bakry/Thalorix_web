...
export default function SellerReviews({
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-white p-6 rounded-2xl border border-gray-100 shadow-sm">
        ...
        <div className="flex flex-col md:flex-row items-center gap-8">
          <div className="text-center shrink-0 py-4 px-6 bg-slate-50 rounded-2xl border border-slate-100/50">
            ...
            <p className="text-xs text-gray-400 font-semibold">{reviews.length} Store reviews</p>
          </div>
      ...
      {/* Write a review form (only if not own profile) */}
      {!isOwnProfile && currentUser && (
        <form onSubmit={handleSubmit} className="bg-white p-6 rounded-2xl border border-gray-100 shadow-sm space-y-4">
          <h4 className="text-lg font-bold text-slate-800 border-b pb-2 border-gray-100">Write Store Review</h4>
          ...
          <div className="space-y-1">
            <label className="text-xs font-semibold text-gray-500">Your Experience / Feedback</label>
            <textarea
              rows={3}
              value={newComment}
              onChange={(e) => setNewComment(e.target.value)}
              placeholder="Tell us about the product code quality, template design or seller support..."
      ...
      {reviews.length > 0 ? (
            ...
            >
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <p className="font-extrabold text-sm text-slate-800">{rev.userId?.name || "Anonymous Customer"}</p>
                    <p className="text-[10px] text-gray-400 font-medium">@{rev.userId?.username || "anonymous"}</p>
                  </div>
                </div>
        ...
        <div className="bg-white py-16 text-center rounded-2xl border border-gray-100 shadow-sm p-6">
          ...
          <p className="text-gray-500 text-sm max-w-sm mx-auto leading-relaxed">
            This store has not received any reviews yet. Be the first to leave a feedback!
          </p>
        </div>
      )}
    </div>
